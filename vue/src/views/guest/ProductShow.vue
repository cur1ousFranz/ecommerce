<template>
  <div>
    <section>
      ...
      <div v-else class="w-10/12 mx-auto py-6 max-h-screen">
        ...
        <div class="flex flex-col  md:flex-row space-x-2">
          ...
          <div class="relative w-12/12 my-4 pr-0 max-h-screen md:overflow-auto md:w-5/12 md:my-0 md:pr-12">
            ...
            <div id="size-container" class="grid grid-cols-5 gap-2 my-3">
              <div v-for="size in model.sizes" :key="size">
                <h1 @click="selectSize(size.name)" :class="model.selectedSize === size.name ? 'cursor-pointer border-2 px-6 w-fit rounded-full border-gray-800' : 'cursor-pointer border px-6 w-fit rounded-full border-gray-400'">
                  {{ size.name }}
                </h1>
              </div>
            </div>
      ...
      <div @click="addToCart(product.id)" class="fixed px-6 bottom-0 w-full">
